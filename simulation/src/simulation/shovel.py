...
def shovel_process(
    ...
    *,
    ...
    unit's model.  Defaults to EX3600 if not supplied.
    """
    if failure_cfg is None:
        failure_cfg = PREMATURE_FAILURE["EX3600"]
    if pm_schedule is None:
        pm_schedule = SHOVEL_PM_SCHEDULE
    ...
    while True:
        ...
        if min_ttf < time_to_next_pm:
            if min_ttf >= time_to_next_pm - OPP_WINDOW_HRS:
                ...
                failed_ag = failed_ag
                needs_bay = True  # premature failure always requires a bay (opportunistic)
                dur_from_ttf = False
            else:
                op_interval = min_ttf
                ...
                needs_bay = AG_REQUIRES_BAY.get(failed_ag, True)
                dur_from_ttf = True
        else:
            op_interval = time_to_next_pm
            ...
            needs_bay = any(pm_schedule[p]["bay"] for p in due_pms)
            dur_from_ttf = False
        ...
        if needs_bay:
            ...
            # No-bay work — mechanic only (on-machine / field).
            req_mech = mechanic_resource.request()
            with (yield req_mech):
                queue_time = env.now - queue_start
                stats.queue_time += queue_time
